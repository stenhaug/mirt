EM_group_simple <- function(pars, constrain, Ls, Data, PrepList, list, Theta, DERIV, solnp_args, control)
{
    # A) Prepare -----------------------------------------------------------------
    ...
    for(g in seq_len(ngroups)){
        for(i in seq_len(J+1L)){
            ...
            estpars <- c(estpars, pars[[g]][[i]]@est) # creates estpars which says which parameters to estimate
        }
    }
    ...
    for (cycles in seq_len(NCYCLES)){
        ...
        Elist <- Estep_simple(pars=pars, Data=Data, gTheta=gTheta, prior=prior, Prior=Prior,
